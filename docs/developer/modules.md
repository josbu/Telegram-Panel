...
任务中心的“复制”按钮只对 `canCreate=true` 且宿主存在专用 Vue 表单的任务类型显示。复制普通批量任务会用原任务配置打开“新建任务”弹窗；复制 Cron 计划任务会同时带入 Cron 表达式和配置。宿主会在前端清理 `items`、`failures`、`recent_failures` 等运行态字段，但模块作者仍应把可复用配置和运行态结果分开存放，避免复制任务时把一次执行的进度或失败明细误认为用户配置。
