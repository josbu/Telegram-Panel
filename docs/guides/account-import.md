...
适用版本：当前开发版起，导入页支持在上传前选择账号分类、自动分配已有 WARP 或创建一对一受管 WARP；Telegram 客户端连接会按账号和 API 类型稳定选择一组匹配的设备参数。
...
选择器、任务详情和风控确认中显示为 `#编号`。批量活跃、批量建群/建频道等任务可在“指定账号编号”
里填写这些编号；支持每行一个，也支持英文逗号 `,`、中文逗号 `，`、顿号 `、` 或分号分隔。内部接口路径和数据库关联仍使用账号 `Id`。删除账号后空出的编号可被后续新账号
复用，因此不要把编号当作长期不可变外部主键。

成功判据是导入/登录成功摘要和账号列表都显示同一个 `#编号`，按 `#编号` 搜索能定位账号，任务表单
填写该编号时不会提示“账号编号不存在或不可操作”。失败排查先确认数据库迁移已执行，且账号仍为启用
状态；回滚方式是恢复到 v1.31.56 或更早版本，旧版会继续使用内部账号 ID，不展示 `#编号`。
