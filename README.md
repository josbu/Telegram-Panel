...
- 🧩 **用户任务：批量订阅/加群**：任务中心新建「用户任务 → 批量订阅/加群」，支持按账号分类筛选；账号列表支持单账号“进群/订阅”入口（多链接换行分隔）
...
说明：这是**用户账号**任务（Bot 无法主动加群/订阅）。

### 入口 1：任务中心批量执行

1. 进入「任务中心」→ 点击「新建任务」
2. 选择：任务分类 =「用户任务」，任务类型 =「批量订阅/加群」
3. 选择执行账号（支持按**账号分类**筛选 + 搜索）
4. 粘贴链接/用户名（**换行分隔**），设置间隔（建议 1500–4000ms）
5. 提交后到「任务中心」查看进度与结果

### 入口 2：账号列表单账号执行

在「账号列表」每行账号的操作区点击「进群/订阅」，输入多个链接/用户名（换行分隔）后确认即可提交后台任务。
